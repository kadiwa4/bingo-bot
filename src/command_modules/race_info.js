			...
			message.multiReply(
				...
				async function* () {
					for (let result of sqlite.getResults.all(race.race_id)) {
						...
						yield `  ${result.forfeited ? game.config.emotes.forfeited : game.placeEmote(placeObject.place)} \`${formatTime(result.time, false)}\` – ${name}${result.load_time ? ` (${formatTimeShort(result.load_time)}\xA0loads)` : ""}\n${members}`;
